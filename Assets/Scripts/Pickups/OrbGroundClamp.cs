...
public class OrbGroundClamp : MonoBehaviour
{
    ...
    void LateUpdate()
    {
        ...
        {
            float bottom = 0f;
            var col = GetComponent<Collider>(); if (col) bottom = col.bounds.extents.y;
            var p = transform.position; p.y = hit.point.y + bottom + skin; transform.position = p;
        }
    }
}
